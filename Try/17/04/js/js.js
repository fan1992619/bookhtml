// JavaScript Document
var datatable = null;//表格元素
  
  //4个参数分别是 数据库名,版本号，数据库的描述，数据库大小
  var db = openDatabase('student','','My Database',102400);//创建数据库对象
  ...
  function showData(row){
      ...
      //填充第一列的信息为该行的num
      td2.innerHTML=row.num;
     //创建第三列，这一列是院系
      var td3=document.createElement('td');
	  td3.innerHTML=row.acade;
	  //创建第四列，这一列是班级
      var td4=document.createElement('td');
	  td4.innerHTML=row.clas;
  ...
  function showAllData(){
    ...
    db.transaction(function(tx){
		var sea=document.getElementById("sea").value;
         ...
         //首先它创建一个数据库表，里面有4个字段
        tx.executeSql('CREATE TABLE IF NOT EXISTS InfoData(name TEXT,num TEXT,acade TEXT,clas TEXT)',[]);
        //创建一个查询语句用来查询数据库表的所有记录（这个由于是所有查询，所以不需要预编译语句和参数 (第二个参数)）
         //然后定义了一个回调函数，表明对于结果集的处理
         tx.executeSql("SELECT * FROM InfoData WHERE num="+sea,[],function(tx,rs){
  ...
  function addData(name,num,acade,clas){
     //开启一个数据库事务
    //回调函数是一个有参数的插入语句，可以看到我们插入到表InfoData中，插入的内容也就是参数传递进来的内容
     db.transaction(function(tx){
         ...
         tx.executeSql('INSERT INTO InfoData VALUES(?,?,?,?)' , [name,num,acade,clas],function(tx,rs){
             console.log("成功保存数据!");
         },
